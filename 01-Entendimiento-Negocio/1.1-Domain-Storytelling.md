...
flowchart LR
    Cliente(["👤 Cliente"])
    App(["📱 App Móvil"])
    Monolito(["🏦 Monolito J2EE"])
    MotorFX(["💱 Motor FX"])
    MotorFraude(["🚨 Motor Fraude"])
    Ledger(["📒 Ledger"])
    Tesoreria(["🏛️ Tesorería"])
    OracleDB(["🗄️ Oracle DB"])
    BancoPartner(["🏦 Banco Partner"])
    
    OpcionEnvio["📋 Opción Enviar Dinero"]
    Transferencia1["💵 Transferencia USD"]
    IDTx["🏷️ ID Transacción"]
    Saldo1["💰 Saldo Cliente"]
    ParCotizacion["💱 Par USD/EUR"]
    TasaCambio["📊 Tasa Cambio"]
    TasaBloqueada["🔒 Tasa 5min"]
    Cotizacion["💶 Cotización EUR"]
    Biometria["👆 Touch ID"]
    OrdenPago["📝 Orden Pago"]
    AnalisisFraude["🔍 Análisis Riesgo"]
    ReglasSQL["📜 Reglas PL/SQL"]
    ScoreAprobado["✅ Score Aprobado"]
    AsientoDebito["📊 Asiento Débito"]
    CuentaCliente["💳 Cuenta Cliente"]
    CuentaPuente["🏦 Cuenta Puente"]
    InstruccionPago["📤 Instrucción Pago"]
    FirmaHSM["🔐 Firma HSM"]
    MensajeSWIFT["📨 SWIFT pacs.008"]
    NotifEnCamino["🔔 Notificación"]
    MensajeEnCamino["🚀 En Camino"]
    ConfirmSWIFT["📬 SWIFT pacs.002"]
    EstadoCompletado["✔️ Estado Completado"]
    AsientoCierre["📊 Asiento Cierre"]
    CuentaLiquidada["💼 Cuenta Liquidada"]
    NotifFinal["🔔 Notificación Final"]
    MensajeFinal["✅ Completada"]
...
flowchart LR
    ...
    SistemaRecon(["🏦 Sistema Reconciliación"])
    OracleDB(["🗄️ Oracle DB"])
    BancosCorresp(["🏛️ Bancos Corresponsales"])
    EquipoOps(["👥 Equipo Operaciones"])
    
    JobRecon["⚙️ Job Reconciliación"]
    ModoReadOnly["🔒 Modo READ-ONLY"]
    SolicitudExtractos["📥 Solicitud Extractos"]
    ArchivosMT940["📄 MT940 SWIFT"]
    ArchivosCAMT["📄 CAMT.053 SEPA"]
    ArchivosJSON["📄 JSON REST"]
    DatosCrudos["📦 Datos Crudos"]
    TablasTemp["🗂️ Tablas CORE_SCHEMA"]
    ProcedureSQL["⚙️ Procedure PL/SQL"]
    JoinMasivo["🔗 JOIN Extractos vs Ledger"]
    Discrepancias["❌ Discrepancias"]
    ReporteExcel["📊 Reporte Excel"]
    EmailsAlertas["📧 Emails Alertas"]
    UpdateMasivo["✔️ UPDATE Masivo"]
    ModoReadWrite["🔓 Modo READ-WRITE"]
    ...
    SistemaRecon -->|13 - Envía| EmailsAlertas
...
flowchart LR
    ...
    OrdenPago["📝 Orden Pago"]
    AnalisisRiesgo["🔍 Análisis Riesgo"]
    ReglaVelocidad["⚡ Regla Velocidad"]
    ReglaGeo["🌍 Regla Geolocalización"]
    ReglaScoring["📊 Regla Scoring"]
    ScoreBajoRiesgo["✅ Score Bajo Riesgo"]
    ProcesamientoPago["💰 Procesamiento Pago"]
    
    Cliente -->|1 - Inicia| Transaccion
    Transaccion --> App
    App -->|2 - Envía| OrdenPago
    OrdenPago --> Monolito
    ...
    Monolito -->|8 - Continúa con| ProcesamientoPago
